...
5. Where a concept is explored interactively in this class's lab in more depth than the lecture needs, add a one-line `<p class="keep-in-present">` pointer ("The bandwidth trade-off below is explored hands-on in the lab →") linking to the lab's relevant anchor, instead of rebuilding the full lab widget on the lecture page. Beyond that, lecture pages should carry their own interactive graphics freely — small, single-purpose widgets (a draggable diagram, a preset-driven recompute, a live counter) that make a central concept concrete, in the same vanilla-JS/SVG style and visual identity as lab widgets. Keep each one scoped to one idea; a lecture widget is a smaller cousin of a lab widget, not a second copy of one.
